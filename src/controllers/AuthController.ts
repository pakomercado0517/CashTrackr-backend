...
export class AuthController {
  ...
  static confirmAccount = async (req: Request, res: Response) => {
    try {
      ...
      user.token= null
      await user.save()
      res.status(200).json({message: "Usuario confirmado, ya puedes iniciar sesión"})
    } catch (error) {
      res.status(500).json({error: error.message})
    }
  }
  ...
  static login = async (req: Request, res: Response) => {
    try {
      ...
      if(!user.confirmed) {
        const error = new Error("Necesitas confirmar tu cuenta para poder iniciar sesión")
  ...
  static forgotPassword = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({message: "Revisa tu email para reestablecer la contraseña"})
    } catch (error) {
      res.status(500).json({error: error.message})
    }
  }
  ...
  static validateToken = async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json({message: "Token correcto, ahora puedes cambiar tu contraseña"})
    } catch (error) {
      res.status(500).json({error: error.message})
    }
  }
