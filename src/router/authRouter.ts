...
router.param('/:userId', validateUserExist);
...
router.post(
  ...
  body('password')
    .isLength({ min: 8 })
    .withMessage('El password es muy corto, mínimo es de 8 caracteres'),
...
router.post(
  ...
  body('newPassword')
    .isLength({ min: 8 })
    .withMessage('La nueva contraseña es muy corta, mínimo es de 8 caracteres'),
  handleInputErrors,
